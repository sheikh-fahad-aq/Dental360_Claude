...
| `src/components/patient-detail/<section>/` | Owned: `appts` 2 · `audit` 2 · `comms` 1 · `contact` 1 · `docs` 5 · `family` 3 · `images` 1 · `journal` 2 · `medical-hx` 12 · `notes` 5 (TipTap) · `post-op` 4 · `schedule` 1 · `tx-plans` **7 files + `builder/` 17** (`EmailTemplateEditor` is **not** among them — it moved to `ui/`) (map in `references/tx-plans.md` §1). **Not owned, in this tree:** `charting/` incl. `perio/` + `treatment-listing/`, `insurance/` 9 + `billing/` 3, `forms/` 5, `labs/` 2. Largest owned: `builder/TreatmentPlanBuilder.jsx` 1805, `docs/DocsSection.jsx` 1537, `appts/AddAppointmentDrawer.jsx` 1334. |
...
- Treatment-plan hard rules. The evidence and the bug each came from are in **`references/tx-plans.md` §3** — read it before changing anything under `tx-plans/`.
  ...
  - **Editability is per ACTION and per LINE, never one flag.** `isPlanEditable` covers `draft|presented|partially_accepted|accepted` (the calendar is the freeze, not the patient's answer); `isPlanItemEditable` refuses a line whose `scheduleStatus` is not `unscheduled`, because a plan stays open while individual lines get booked. In the builder footer only **Recalculate** is a content edit — Present, Send, Mark response and Schedule are permitted on a signed, accepted plan and must stay available, since scheduling is the next thing that happens to one. A signed plan is unlocked by **"Start a new version"** (`reopenTreatmentPlan`), which detaches the signature and clears the answers; the signed text survives as the revision captured when it was signed. Never say "Void it" — this app has no Void affordance.
  - **A SETTLED plan's action row is hidden, not disabled.** `scheduled`, `completed` and a viewed revision leave only **Close** and the **⋯** menu (both read-only: Print, PDF, Share link, Compare, Duplicate, Archive). The greyed-with-a-reason reading earns its place only when the action exists but is unavailable — on a settled plan there is no fee to re-price, nobody to present to, no response outstanding and the booking already exists, so six dead buttons just bury Close. `void` and `declined` deliberately keep theirs disabled: a declined plan can be presented again, and void is the case where the reason is what the operator needs.
  ...
  - `EmailTemplateEditor` edits a **body only** and now lives in `src/components/ui/`, shared with the Add Appointment notify composer — its banner, button label and variable list are props. `buildTreatmentPlanEmailHtml` inlines styles, leaves `[Bracketed]` variables for the server, and nothing assigns `innerHTML`.
...
- **Navigation state is consumed then WIPED.** `PatientDetail.jsx` seeds the patient from
  `location.state.seedPatient` and immediately `navigate(..., { replace: true, state: {} })`.
  Anything else arriving that way must be captured on the FIRST render or it is gone — which is why
  `initialChartVisitId` (Scheduling's Open Chart → `fe-charting`) is read with a lazy `useState`
  initialiser and handed to `ChartingProvider`, not read at the point of use.
